...
```powershell
& ([scriptblock]::Create((Invoke-RestMethod "https://raw.githubusercontent.com/DavidDiaz0317/hive/RELEASE_TAG/v2/install-integrated.ps1")))
```
...
curl --fail --silent --show-error --location \
  https://raw.githubusercontent.com/DavidDiaz0317/hive/RELEASE_TAG/v2/install-integrated.sh | sh
```

The installer verifies the archive checksum, GitHub Sigstore build attestation, platform, and every file in `distribution-manifest.json` before atomically activating it. Replace `RELEASE_TAG` with the selected immutable release tag. The install fails closed if `gh attestation verify` fails.
