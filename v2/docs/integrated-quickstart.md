...
The inspection selects `npm`, `pnpm`, or Yarn from the nearest committed ancestor lockfile for each package root and rejects ambiguous same-scope locks. The installer handles independent lockless package roots separately, avoids reinstalling children owned by an ancestor lock, and chooses the immutable/frozen Yarn flag from the resolved Yarn major version. For comprehensive coverage, Hive prefers a repository-authored aggregate such as `test:all` only when a strict unconditional `run SCRIPT && ...` graph reaches every selected terminal test. The only semantic substitution is a Vitest coverage leaf whose normalized command is byte-for-byte the unit leaf after removing coverage-only flags. Hive then runs that aggregate once instead of duplicating its browser and smoke suites; if proof fails, it retains the explicit commands. The generated workflow records every repository command and its first nonzero exit in `.visual-hive/repository-tests.tsv` and `.visual-hive/repository-test-exit-code.txt`, continues into Visual Hive so review evidence is still uploaded, and makes the required PR check fail on either the repository plan or deterministic pipeline verdict. A production bundle from an incomplete repository test plan may still open or update evidence-backed findings, but it is not authoritative for resolving an absent finding.
