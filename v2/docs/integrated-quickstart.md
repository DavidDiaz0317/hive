...
Windows release executables are additionally Authenticode-signed with an Azure Artifact Signing Public Trust identity before `hive-dist` hashes them into the distribution manifest. This is transparent to installers. Fork release maintainers configure the OIDC-bound identity by following [integrated Windows release signing](integrated-windows-signing.md); an absent or invalid signing identity stops publication rather than producing an unsigned Windows archive.
