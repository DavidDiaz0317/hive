...
func verifyCodexPlatformContainment(ctx context.Context, codexCommand string) error {
	...
	defer cleanupCodexContainmentProbe(root, binRoot, blockedRoot, readPath, writePath, probePath)
...
func runCodexContainmentProbe(ctx context.Context, codexCommand, cwd, probePath, mode, target, address, expectedMarker string) error {
	...
	command.Dir = cwd
	command.Env = containmentProbeEnvironment(mode, target, address)
	...
	if err != nil || strings.TrimSpace(stdout.String()) != expectedMarker {
		return fmt.Errorf("Codex %s containment probe was not denied by the active no-files profile: %s", mode, containmentProbeFailureDetail(err, output))
	}
	return nil
}
...
func containmentProbeEnvironment(mode, target, address string) []string {
