...
func runCodexProviderInvocationHelper(args []string) {
	if len(args) > 0 && args[0] == "sandbox" {
		...
		readableRoot := filepath.Clean(optionValue(args, "--sandbox-state-readable-root"))
		if target := strings.TrimSpace(os.Getenv(containmentProbeTargetEnv)); target != "" {
			target = filepath.Clean(target)
			relative, err := filepath.Rel(readableRoot, target)
	...
	if argumentSequencePresent(args, "features", "list") {
		...
		if output == "" {
			output = codexReviewedFeatureInventoryV0144
		}
		fmt.Println(output)
		os.Exit(0)
	}
...
func TestCodexProviderRejectsFeatureInventoryDrift(t *testing.T) {
	tests := []struct {
		...
		{"unknown feature", codexReviewedFeatureInventoryV0144 + "\nnew_unreviewed_tool stable true", `unreviewed feature "new_unreviewed_tool"`},
		{"missing feature", strings.Replace(codexReviewedFeatureInventoryV0144, "shell_tool                           stable             true\n", "", 1), `omitted reviewed feature "shell_tool"`},
		{"semantic drift", strings.Replace(codexReviewedFeatureInventoryV0144, "shell_tool                           stable             true", "shell_tool                           stable             false", 1), `feature "shell_tool" changed`},
		{"malformed output", codexReviewedFeatureInventoryV0144 + "\nnot a valid feature row", "feature list line"},
	}
